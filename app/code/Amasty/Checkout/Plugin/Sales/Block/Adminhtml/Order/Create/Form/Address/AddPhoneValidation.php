...
class AddPhoneValidation
{
    ...
    public function afterGetForm(Address $subject, Form $result): Form
    {
        ...
        if (!in_array($elementName, $this->processedNames, true)) {
            // Ensure a string is always passed to the processor even if getClass() returns null.
            $currentClass = (string) ($phoneElement->getClass() ?? '');
            $phoneClass = $this->adminPhoneValidationProcessor->process($currentClass);

				...
				if($_SERVER['REQUEST_METHOD'] === "POST"){
					if ($type == 'TRANSACTION') {
						...
						if (
							...
							$amount_cents = $obj['amount_cents'] / 100;
							$currency = $order->getOrderCurrencyCode();
							$Accepttransaction = $obj['id'];
							$Acceptorder = $obj['order']['id'];

							if(isset($data['data']['total_fees']))
							{
								$fees=$data['data']['total_fees'];
								$order->addStatusHistoryComment(__("Order Admin Fees: $fees"))->save();
							}
							if(isset($data['data']['mainAdminFees']))
							{
								$feesmainAdminFees=$data['data']['mainAdminFees'];
								$order->addStatusHistoryComment(__("Order Admin Fees: $feesmainAdminFees"))->save();
							}
							if(isset($data['data']['downpayment']))
							{
								
								$feesdownpayment=$data['data']['downpayment'];
								$order->addStatusHistoryComment(__("Order Down payment: $feesdownpayment"))->save();
							}

							if( isset($data['data']['down_payment']) ){
								$down_payment = $data['data']['down_payment'];
								if(isset($data['data']['currency']) &&!empty($data['data']['currency']))
								{
									$currency = $data['data']['currency'];
									$order->addStatusHistoryComment(__("Order Down payment: $down_payment $currency"));
								}
								else
								{
									$currency = 'EGP';
                                ...
                                $order->addStatusHistoryComment(__("Accept Order ID ($Acceptorder)."));
                                $order->addStatusHistoryComment(__("Accept Transaction ID ($Accepttransaction)."))
                                    ->save();
							}else{
							...
							$invoice->setGrandTotal($grandTotal)
											...
											->pay()
											->save();
							$order->setTotalPaid($amount_cents)->save();
							...
							try {
								...
								$order->setState(\Magento\Sales\Model\Order::STATE_CLOSED)
											->setStatus(\Magento\Sales\Model\Order::STATE_CLOSED)->save();
							...
							$order->setState('incomplete_transaction')
								  ->setStatus('incomplete_transaction')->save();
                        ...
                        $customerData = $this->customer->setWebsiteId($this->website_id)->loadByEmail($email);
						if($customer_id)
						{
                    ...
                    $orderID = substr($_GET['merchant_order_id'], 0, -11);
					...
					if($data['success'] == 'true') {
						...
						if(isset($data['data_message'])&& !empty($data['data_message']))
						{
							$this->messageManager->addSuccess(__($data['data_message']));
						}
						else
						{
							$this->messageManager->addSuccess(__('Payment Approved'));
						}
                        ...
                        if ($order->getId() && $order->getState() != \Magento\Sales\Model\Order::STATE_CANCELED) {
							if(isset($data['data_message'])&& !empty($data['data_message']))
							{
								$order->registerCancellation($data['data_message'])->save();
							}
							else
							{
								$order->registerCancellation('Payment Declined')->save();
							}
                        ...
                        $response_url = $this->base_url . 'checkout/cart';
						if(isset($data['data_message'])&& !empty($data['data_message']))
						{
						...
						{
							$this->messageManager->addError(__('Payment Declined'));
						}

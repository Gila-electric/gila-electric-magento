...
class Rewardproduct extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
    ...
    {
        $productMetadata = $this->_metadataPool->getMetadata(RewardproductInterface::class);
        if (!is_numeric($value) && $field === null) {
            $field = 'identifier';
        } elseif (!$field) {
            $field = $productMetadata->getIdentifierField();
        }
        $entityId = $value;

        if ($field != $productMetadata->getIdentifierField()) {
            $select = $this->_getLoadSelect($field, $value, $object);

            $select->reset(Select::COLUMNS)
                ->columns($this->getMainTable() . '.' . $productMetadata->getIdentifierField())
                ->limit(1);

...
namespace Webkul\RewardSystem\Ui\Component\Listing;
...
class CustomerColumns extends \Magento\Ui\Component\Listing\Columns
{
    /**
     * @var int
     */
     ...
     * @param ColumnFactory       $columnFactor
     * @param AttributeRepository $attributeRepository
     * @param InlineEditUpdater   $inlineEditorr
     * @param array               $components
     * @param array               $data
     */
    public function __construct(
        ContextInterface $context,
        ColumnFactory $columnFactor,
        \Magento\Customer\Ui\Component\Listing\AttributeRepository $attributeRepository,
        InlineEditUpdater $inlineEditorr,
        array $components = [],
        array $data = []
    ) {
        parent::__construct($context, $components, $data);
        $this->columnFactory = $columnFactor;
        $this->attributeRepository = $attributeRepository;
        $this->inlineEditUpdater = $inlineEditorr;
    }
    ...
    public function prepare()
    {
        ...
        foreach ($this->attributeRepository->getList() as $newAttributeCod => $attributeData) {
            if ($newAttributeCod!='dob') {
                if (isset($this->components[$newAttributeCod])) {
                    $this->updateColumn($attributeData, $newAttributeCod);
                } elseif (!$attributeData[AttributeMetadata::BACKEND_TYPE] != 'static'
                    && $attributeData[AttributeMetadata::IS_USED_IN_GRID]
                ) {
                    $this->addColumn($attributeData, $newAttributeCod);
                }
            }
        }
